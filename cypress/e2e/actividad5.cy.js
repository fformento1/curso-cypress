...
describe('Actividad complementaria 5', () => { 
    ...
    it.only("Deberia validad la primera espera", () => {
			// Verificar que el primer mensaje que aparece a los 10 segundos luego de 5 segundos mas sea diferente. (que cambie el mensaje) ('have.text') 2 lineas de codigo
            cy.get('#message', {timeout: timeout}).should('have.text', 'You have waited for ten seconds, CONGRATULATIONS');
            cy.get('#message', {timeout: timeout * 1.5}).should('not.have.text', 'You have waited for ten seconds, CONGRATULATIONS')
	})
 })